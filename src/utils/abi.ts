...
export const fetchContractABIFromEtherscan = async (
  verifiedContractAddress: Address,
  chainId: number
) => {
  ...
  if (implementation && !isZeroAddress(implementation)) {
    ...
    if (abiData.status === "1" && abiData.result) {
      ...
      console.error(
        "Error fetching ABI for implementation from Etherscan:",
        abiData
      );
